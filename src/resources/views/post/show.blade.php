...
@section("content")
    <div class="flex w-full justify-between self-start py-12">
        <div class="max-w-3xl">
        ...
        {{-- recent posts --}}
        <div>
            <h2 class="mb-4 text-2xl font-semibold">recent posts:</h2>

            @foreach ($recentPosts as $recent)
                <div class="mb-3 space-y-1 pb-3">
                    <a
                        href="{{ route("post.show", $recent->id) }}"
                        class="font-semibold text-blue-500"
                    >
                        {{ $recent->title }}
                    </a>
                    <p class="text-sm">
                        {{ Str::limit($recent->excerpt, 60) }}
                    </p>
